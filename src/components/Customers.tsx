...
export const Customers = () => {
    ...
    const handleWhatsApp = (customer: Customer) => {
        ...
        const msg = smsTemplates.serviceReminder(
            customer.name,
            veh?.make || 'your vehicle',
            veh?.model || '',
            veh?.license_plate || '',
            tenant?.name || 'our workshop',
            tenant?.phone || ''
        );
    ...
    const handleOpenSmsModal = (customer: Customer) => {
        if (!customer.phone) {
            toast("No phone number for this customer.", 'warning');
            return;
        }
                        ...
                        <div key={customer.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-colors">
                            <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6 gap-4">
                                ...
                                <div className="flex gap-2 w-full md:w-auto justify-end flex-shrink-0">
                                    <button
                                        type="button"
                                        onClick={() => handleOpenSmsModal(customer)}
                                    ...
                                    <button
                                        ...
                                        className="p-2 bg-slate-800 rounded-lg text-[#25D366] hover:bg-[#25D366]/20 transition-colors flex items-center justify-center"
